...
from layers.block import TransformerEncoder
...
class PatchTSTEncoder(nn.Module):
    def __init__(self, patch_len, num_patch,
        ...
        self.encoder = TransformerEncoder(embed_size, hidden_size, heads=n_head, num_layers=n_layer,
    ...
    def forward(self, x):
        ...
        x = torch.reshape(x, (batchsize * n_feature, num_patch, patch_len))     # [batchsize * n_feature, num_patch, patch_len]
        token_emb = self.lin_pro(x)    # [batchsize * n_feature, num_patch, embed_size]
        pos_emb = self.pos_emb(x)
        x = self.dropout(token_emb + pos_emb)
        x = self.encoder(x)     # [batchsize, n_feature, num_patch, embed_size]
...
class PatchTST(nn.Module):
    def __init__(self, seq_len, n_feature, patch_len, stride,
        ...
        self.patch_encoder = PatchTSTEncoder(patch_len, self.num_patch, embed_size, hidden_size,
    ...
    def forward(self, x):
        ...
        encoded_x = self.patch_encoder(masked_x)                        # [batchsize, n_feature, num_patch, embed_size]
        u = self.pro_head(encoded_x)                               # [batchsize, n_feature, num_patch, patch_len]

        return x_patch, u, mask, encoded_x


class PatchTSTPred(nn.Module):
    def __init__(self, backbone: PatchTST, pred_len):
        super().__init__()
